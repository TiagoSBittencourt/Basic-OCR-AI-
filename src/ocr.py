...
class OCRNeuralNetwork:
    ...
    def __init__(self, num_hidden_nodes, data_matrix, data_labels, train_indices, use_file=True):
        ...
        if self._use_file and os.path.exists(OCRNeuralNetwork.NN_FILE_PATH):
            ...
            self.theta1 = self._rand_initialize_weights(784, num_hidden_nodes)  # 28x28 = 784
            self.theta2 = self._rand_initialize_weights(num_hidden_nodes, 10)   # 10 output digits
    ...
    def train_on_instance(self, data):
        ...
        hidden_errors = np.multiply(np.dot(self.theta2.T, output_errors), self.relu_prime(y1).reshape(-1, 1))

        self.theta1 += self.LEARNING_RATE * np.dot(hidden_errors, y0.reshape(1, -1))
